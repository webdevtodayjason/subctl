...
|---|---|---|
| TypeScript (Evy + dashboard) | `components/master/policy/exec.ts` (re-exported via `components/master/policy/index.ts`) <!-- legacy code identifier — renamed in Phase 3 --> | `execCommand`, `execCommandGated`, `PolicyDenied` |
...
| Site | Variant | Reason |
|---|---|---|
| `lib/master.sh` — `launchctl bootout "$target"` | `subctl_exec` | Evy daemon kick. Operator-typed CLI verb. |
| `lib/master.sh` — `launchctl bootstrap "gui/$uid" "$plist"` | `subctl_exec` | Evy daemon kick (paired with bootout). |
...
- `dashboard/server.ts:3176/3178/3182/3433/3435/3439/4209/4211/4215` — `launchctl unload/load/start` + `pgrep` for Evy bounce. Static. (PR 11.5)
...
- `components/master/tools/system.ts:22` — the lone `execSync` in Evy. EXEC_SURFACE §4h — shell-interpolated `name`/`path`/`codeRoot`. Highest-priority migration target; needs `execSync` → `execCommand` AND argv-array refactor in the same change. (PR 11 or its own PR)
...
- `lib/master.sh:129-415` — many `curl` calls to Evy's HTTP surface. Internal HTTP, not exec.
