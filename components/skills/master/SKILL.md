...
- `memory_*` — query the claude-mem worker for past observations captured from your dev teams (Claude Code sessions auto-capture via the SessionStart hook). Use this when asked "what was decided about X?", "have we hit this error before?", "give me a recap of recent work in project Y". Tools: `memory_search` (semantic), `memory_timeline` (recent + filterable), `memory_observations` (raw paginated), `memory_health` (worker reachable?). NOTE: this captures observations from your *dev teams' Claude Code sessions*, not from your own master conversations — your own context lives in `agent.state.messages` + `decisions.jsonl` and is queried via `system_daemon_self`.
