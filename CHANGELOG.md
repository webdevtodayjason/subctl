...
### `feat(dashboard): v2.7.34 operator-facing policy UI — preset application, chip-list resolved view, form editors`

The policy engine has shipped since v2.7.0, but everything operator-facing was either CLI (`subctl policy list/explain/audit`) or TOML-by-hand. The dashboard surface added in v2.7.0 PR 11 showed *audit data* (recent denials, verifier interventions, per-team allowlist sha) but rendered the resolved policy itself as a raw JSON dump, and offered no path at all to *edit* the policy without dropping into `$EDITOR`. Operators with multiple projects (this operator: 150+) hit the same friction every project: write the policy file by hand, validate, re-validate, re-spawn. This release pulls forward the v2.9.x roadmap's "operator-facing policy UI wave" into v2.7.34 because it's independent of v2.8.0 team templates and the friction is real now.

**Scope 1 — Apply-preset action per project.** The Projects tab's per-project detail panel grows an `Apply preset…` dropdown next to "Spawn dev team" / "Open in Vault Viewer". Preset list is populated lazily on first project view from `GET /api/policy/presets` (cached on `window.__policyPresetsCache`). Selecting a preset prompts for confirmation, then `POST /api/policy/preset/<project> {preset}` writes a fresh `<project>/.subctl/policy.toml` whose entire body is the one-liner `preset = "<name>"`. The preset's allowlist is inherited via the existing merge chain — no inline copy of the preset's rules into the project file, so updates to the shipped preset reach the project on next spawn. Future workers spawned for the project pick up the preset's allowlist on their next spawn snapshot.

**Scope 2 — Resolved-policy chip-list view.** The Resolved-policy panel on the Policy tab used to dump `JSON.stringify(policyDoc, null, 2)` into a `<pre>`. v2.7.34 swaps in a chip-list renderer that groups the resolved allowlist into five visual categories: Allowed commands (green chips), Allowed patterns (blue chips), Ecosystem allowlists (gold chips), Deny substrings (red chips), Deny regex (pink chips). Each chip shows an origin tag (`project` / `user` / `preset:<name>` / `defaults`) derived from the snapshot's source_paths metadata, and hovering reveals the full rule body + rule-path + origin via the chip's `title` attribute. A view-toggle button in the panel header switches back to the JSON dump for power users who need the raw output. The chip-list data shape is exposed via `GET /api/policy/resolved/<team_id>` (re-resolves from disk; does not read the frozen snapshot body) so the dashboard's polling refresh shows the *current* policy that future workers will pick up, not the snapshot any one running worker is gated by.

**Scope 3 — User policy editor.** New `Policy editor` panel on the Policy tab carries three sub-tabs. The first is a form-based editor over `~/.config/subctl/policy.toml` with sections for top-level scalars (preset, default_mode), `mode.gated.allow.commands` (add/remove list), `mode.gated.allow_pattern` (per-row: command + comma-separated args + comma-separated `deny_if_arg_contains`), `mode.gated.deny_always.substrings` (add/remove list), and `mode.gated.deny_always.regex` (add/remove list). `GET /api/policy/user` reads the file (returns empty doc shape if the file doesn't exist); `POST /api/policy/user {doc}` writes it. The POST handler validates the TOML and the shape before writing — invalid top-level keys, invalid `default_mode` value, non-array `allow_pattern` — all return 400 with a precise `field: message` error. The form's "save" button surfaces success (`saved (N bytes)`) or the validation error inline.

**Scope 4 — Project policy editor.** Identical UI shape to the user editor, scoped per-project. Project dropdown enumerated from the existing `/api/projects` scanner so the list matches what the Projects tab shows. `GET /api/policy/project/<project>` reads `<project>/.subctl/policy.toml`; `POST /api/policy/project/<project> {doc}` writes it. Project segment can be either a bare directory name (resolved under `SUBCTL_CODE_ROOT`, `~/code` by default) or an absolute path, but absolute paths are refused unless they live under either the code root or `$HOME` — this surface is never a "write to any path on disk" primitive. Path-traversal is refused at the validator (`..`, `\0`, or anything outside `[A-Za-z0-9._/~-]`).

**Scope 5 — API surface.** All handlers live in the new pure-function module `dashboard/lib/policy-api.ts` (mirrors the testability shape of `dashboard/lib/audit-api.ts` shipped in v2.7.0 PR 11). Server.ts gets ~50 lines under the `// ── v2.7.34 policy UI ──` marker, all minimal wrappers around the pure handlers. The four routes plus two reads: `GET /api/policy/presets` (preset name enumeration), `GET|POST /api/policy/user`, `GET|POST /api/policy/project/:project`, `POST /api/policy/preset/:project {preset}`, `GET /api/policy/resolved/:team_id` (chip-list shape keyed off a running team's snapshot project_root), `GET /api/policy/resolved-project/:project` (same shape keyed off a project directly — handy from the project editor for live-preview after save). The full resolved policy shape (`{ok, project_root, mode, preset, allowlist_sha, source_paths, resolved_at, chips: [{kind, label, origin, rule_path, detail}]}`) is documented inline at the top of `policy-api.ts`.

**Tests.** `dashboard/__tests__/policy-api.test.ts` covers 29 cases: `validatePolicyShape` accepts/rejects the documented shapes (empty doc, minimal valid doc, unknown top-level key, invalid default_mode, non-array allow_pattern, unknown gated key); `resolveProjectFromName` resolves bare names + refuses traversal + refuses paths outside code root + 404s on missing; user policy GET/POST round-trip (empty case, full round-trip, invalid TOML rejected, schema-invalid rejected); project policy round-trip + traversal refusal; apply-preset writes the expected one-line policy + rejects unknown preset + 400s on missing name; `listAvailablePresets` enumerates correctly; `chipListFromResolved` emits the right chip kinds (command + pattern + deny + deny_regex + ecosystem) with correct labels + rule_paths; end-to-end `handleResolvedForProject` returns the chip list with the project's own rule + preset rules + defaults' deny chips; TOML round-trip helpers parse empty + handle non-trivial docs + strip `__meta`.

**Files:**

- New: `dashboard/lib/policy-api.ts` — pure handlers, ~600 lines, mirrors the audit-api module
- New: `dashboard/__tests__/policy-api.test.ts` — 29 tests, 73 expect() calls
- Modified: `dashboard/server.ts` — ~50 lines under the v2.7.34 marker, route wiring only
- Modified: `dashboard/public/index.html` — chip-view toggle on resolved panel + 3-tab policy editor panel
- Modified: `dashboard/public/app.js` — chip renderer + form editor wiring + per-project Apply-preset dropdown
- Modified: `dashboard/public/style.css` — chip styles (5 colors) + editor form styles + apply-preset dropdown
- Modified: `dashboard/package.json` — `smol-toml` ^1.6.1 dep added (was transitively available via master; now declared because policy-api.ts imports it directly)
- Modified: `docs/policy.md` — new "Operator UI" section walking through the four surfaces
- Modified: `VERSION` → `2.7.34`

## [2.7.32] — 2026-05-13

### `chore(cleanup): v2.7.32 watchdog reconciliation + CLI PATH + tmux SSH + /health cleanup + stale-team gc + CHANGELOG sort`

Six small fixes bundled before the v2.8.0 work begins. Theme: cleanup of edges that surfaced during the v2.7.25–v2.7.31 parallel ship-fest.

**Scope 1 — Watchdog reconciliation (operator-flagged 2026-05-13).** The team-staleness watchdog (shipped v2.7.19, refactored v2.7.22) tracked teams via the in-memory `teamLastActivity` map seeded from tmux sessions + inbox events. When an operator archived a team's registry dir to `~/.local/state/subctl/teams/.killed/` (e.g. `claude-osint-cve-monitor`), the tracker still kept the entry and the watchdog kept nudging/alerting on a team that no longer existed. Tmux-session pruning didn't fire because the session itself could outlive the registry archive. Fix: `runStaleTeamSweep` (`components/master/auto-nudge.ts`) now accepts an optional `teamRegistryExists` predicate + `emitVanished` callback. Before deciding nudge/alert for each tracked team, it checks the on-disk registry; if absent → action `"vanished"`, one-shot `severity:"alert", kind:"team-vanished"` notification fires, state entry is cleared, server.ts also drops the team from `teamLastActivity` + `teamPaneHash` so the alert is genuinely one-shot. Tests in `components/master/__tests__/auto-nudge.test.ts` pin the four cases: vanished fires correctly, second-sweep-with-empty-tracker stays silent (no spam), predicate-omitted preserves pre-v2.7.32 behavior, predicate-returns-true takes the normal path.

**Scope 2 — `subctl` CLI install path.** v2.7.28 shipped the bash dispatcher at `bin/subctl` but `install.sh`'s fallback target was `$HOME/bin/` (which only works if the operator's dotfiles already exported it), and the post-install warning didn't probe `$PATH` so operators couldn't tell whether the symlink would resolve. Fix: fallback target moved to `$HOME/.local/bin/` (XDG standard, auto-included on recent macOS/zsh setups). After symlinking, the installer probes the current shell's `$PATH` for the target dir — if absent, it prints the exact `export` line to add to `~/.zshrc`. `/usr/local/bin/` remains the first-choice target (no sudo prompt path); only used when writable. `docs/cli.md` documents the new fallback + PATH requirement.

**Scope 3 — tmux PATH for SSH.** Operator hit `tmux: command not found` running `ssh host subctl …` because non-interactive SSH shells don't source `~/.zshrc` (where Homebrew's `/opt/homebrew/bin` export typically lives). Two-layer fix: (1) daemon side — `components/master/server.ts` now resolves `tmux` to an absolute path at boot via a candidate walk (`/opt/homebrew/bin/tmux` first on Apple Silicon, falling through `/usr/local/bin/`, `/usr/bin/`, `/bin/`, then PATH); override with `SUBCTL_TMUX_BIN`. All three internal tmux invocations (diag probe, `list-sessions`, `capture-pane`) use the resolved path. (2) operator side — `docs/master.md` documents adding `export PATH="/opt/homebrew/bin:$PATH"` to `~/.zshenv` (which IS sourced by non-interactive SSH shells, unlike `~/.zshrc`). Operators driving subctl only through the CLI don't need step 2 — the daemon-side fix resolves tmux internally.

**Scope 4 — `/health` field cleanup.** Prior to v2.7.18 `/health` returned `supervisor_model: <id>`. v2.7.18 replaced it with `active_profile` (a name like `"chat"`, not the model id), which made the operator's "what model is actually running?" question require a second `/profile` fetch. v2.7.32 restores BOTH fields — `active_profile` (profile name) and `supervisor_model` + `supervisor_provider` (resolved from `supervisorCfg.model` / `supervisorCfg.provider`, reassigned on every profile swap so it's always live). No tests depended on the absent field; no field is being removed.

**Scope 5 — Stale CLI test team dirs.** `~/.local/state/subctl/teams/` accumulates one dir per `subctl orch spawn` (HMAC secret + policy snapshot). Dirs aren't cleaned up when the tmux session dies because re-spawn reuses the HMAC. CLI test runs (`policy-fix-verify`, `ship-verify-test`, etc.) left dirs behind forever and the watchdog could pick them up. New module `components/master/team-gc.ts` runs a startup GC sweep: any team dir whose `policy.snapshot.toml` mtime > 14 days AND whose `audit/<team_id>.jsonl` mtime > 7 days (or is absent) → moved to `teams/.killed/<team_id>/`, `severity:"info", kind:"team-gc'd"` notification fires, decision logged. Archival, not deletion — operators can still recover the HMAC secret from `.killed/<team_id>/hmac.secret`. Runs synchronously during master boot, BEFORE the team-staleness ticker arms, so the watchdog never starts tracking a dir that's about to vanish. Tunable via `SUBCTL_STATE_DIR`. Tests in `components/master/__tests__/team-gc.test.ts` cover the archive predicate, the keep predicate (fresh snapshot OR recent audit), the no-snapshot skip case, idempotency (re-running on an existing archive coexists via timestamp-suffix), and the `.killed/` self-protection.

**Scope 6 — CHANGELOG ordering pass.** Tonight's parallel ship-fest produced two structural defects in `CHANGELOG.md`: (a) the file's `# Changelog` header was lost during a merge; (b) the v2.7.25 / v2.7.27 / v2.7.29 entries ended up in a duplicated second copy of the entire history appended after `0.1.0`. The v2.7.28 entry was missing from the main file entirely (only present in the v2.7.28-cli worktree). Rebuilt the file end-to-end: restored the file header, deduplicated by version (first occurrence wins), re-sorted descending semver, recovered the v2.7.28 entry from the worktree and slotted it correctly. Net: 68 unique version entries, no duplicates, reverse-chronological top-to-bottom.

**Files:**

- New: `components/master/team-gc.ts` — startup GC sweep + decision API
- New: `components/master/__tests__/team-gc.test.ts` — 9 tests
- `components/master/auto-nudge.ts` — `teamRegistryExists` predicate + `emitVanished` callback + `"vanished"` action kind
- `components/master/__tests__/auto-nudge.test.ts` — 4 new tests covering reconciliation, dedup, opt-in predicate
- `components/master/server.ts` — wires team-gc into boot, wires `teamRegistryExists`/`emitVanished` into watchdog sweep, drops vanished IDs from `teamLastActivity`+`teamPaneHash`, resolves tmux to absolute path, adds `supervisor_model` + `supervisor_provider` back to `/health`
- `components/master/notifications.ts` — adds `team-vanished` + `team-gc'd` to the known-kinds doc comment
- `install.sh` — fallback target `$HOME/bin` → `$HOME/.local/bin`; post-link PATH probe with exact `export` line on miss
- `docs/cli.md` — install-path section rewritten with v2.7.32 callout + PATH requirement
- `docs/master.md` — new sections "5.2.0 /health fields", "5.2.1 Startup team-dir GC", "5.2.2 tmux PATH over SSH"
- `CHANGELOG.md` — full reorder + dedup + header restoration + v2.7.28 entry recovered from worktree
- `VERSION` → `2.7.32`

**Constraints satisfied:**

- `claude_code` outside `providers/claude/`: zero hits in NEW code
- No secrets logged
- `bun test` clean for the new test files (13 auto-nudge tests, 9 team-gc tests, all green)
- `bun build` of `components/master/server.ts` succeeds
