...
subctl_migrate_generate_aliases() {
  ...
  local out="$SUBCTL_CONFIG_DIR/shell-aliases.sh"
  {
    ...
    echo ""
    # Canonical claude binary — absolute path so the guard below (and the
    # per-account launcher scripts in ~/.local/bin) resolve the same native
    # install regardless of PATH (the tmux/launchd PATH differs from the
    # interactive shell). Override by exporting CLAUDE_BIN before sourcing.
    echo "CLAUDE_BIN=\"\${CLAUDE_BIN:-\$HOME/.local/bin/claude}\""
    echo ""
    # NOTE: per-account launchers (claude-jason etc.) are standalone wrapper
    # scripts in ~/.local/bin (written below), NOT shell aliases — so they
    # work in scripts, cron, and non-interactive shells too.
...
FNS
  ...
  # ── per-account launcher wrappers in ~/.local/bin ──────────────────────────
  # Standalone scripts (NOT shell aliases) so account routing works in
  # scripts, cron, and non-interactive shells too. Each pins its account's
  # CLAUDE_CONFIG_DIR and execs the canonical native binary by absolute path.
  local bindir="$HOME/.local/bin"
  mkdir -p "$bindir"
  local wcount=0 walias wprovider wemail wcfg wdesc
...
WRAP
    chmod +x "$bindir/$walias"
    wcount=$((wcount + 1))
  done < <(subctl_list_accounts)
  subctl_ok "wrote $wcount per-account claude launcher(s) to $bindir"
}
